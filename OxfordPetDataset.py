...
class OxfordIIITPet(VisionDataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any]:
        ...
        mask = self.preprocess_mask(np.array(Image.open(os.path.join(self._segs[idx])).convert("L"), dtype=np.float32))
        # image = cv2.imread(image_filename)
        # image = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
        # mask = cv2.imread(os.path.join(self._segs[idx]), cv2.IMREAD_UNCHANGED)
        ...
        #         target.append(np.expand_dims(cv2.imread(os.path.join(self._segs[idx]), cv2.IMREAD_GRAYSCALE),axis=0))
        # if not target:
        #     target = None
        # elif len(target) == 1:
        #     target = target[0]
